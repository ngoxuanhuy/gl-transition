...
bool in_inside_lozenge(vec2 p){
  vec2 vertex1, vertex2, vertex3, vertex4;
  vertex1 = vec2(0.5, -0.25+progress);
  vertex2 = vec2(-0.25+progress, 0.5);
  vertex3 = vec2(0.5, 1.25-progress);
  vertex4 = vec2(1.25-progress,0.5);
...
bool in_outside_lozenge(vec2 p)
{
  vec2 vertex1, vertex2, vertex3, vertex4;
  vertex1 = vec2(0.5, -0.25-progress);
  vertex2 = vec2(-0.25-progress, 0.5);
...
vec4 transition (vec2 uv) {
  if (progress <= 0.75)
  {
    if (in_inside_lozenge(uv))
    {
      return getFromColor(uv);
    }
    else if (!in_outside_lozenge(uv))
    {
      return getFromColor(uv);
    }
